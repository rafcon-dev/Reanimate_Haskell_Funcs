...
module R_minimum (animation) where
...
minFound = scale 0.5 $ withColFromToPixel funcColorPixel 0 end $ center $ latexCfg calligraCfg $ L.pack minFoundStr
    where
        end = (length minFoundStr) -1
        
moreThanQuestion = mkGroup
                    [
...
animation = env $ scene $ do
    ...
    obj_minFound <- oNewWithCam minFound cam
    
    oModifyS obj_minFound $ oTranslateX .= firstElX >> oTranslateY .= 0 >> oScale .= 0
    oShow obj_minFound
    ...
    waitOn $ forkAllWithDifferentLags [0.1,0.0,0]
        ...
        ,oMoveToAndScale obj_minFound (firstElX, 1.7) 1 0.5
        ]
     
    
    let obj_array_withOriginalArray = zip obj_values inputArr
        
    waitOn $ findMinimumAnimation (head obj_values) obj_op (tail obj_array_withOriginalArray) (head inputArr)
   ...
   --     ]
    ...
    obj_result <- oNewWithCam (counter $ minimum inputArr) cam
    let min_Index = indexOfMinimum inputArr
    resultX <- oRead (fst $ obj_array_withOriginalArray!!min_Index) oTranslateX
    resultY <- oRead (fst $ obj_array_withOriginalArray!!min_Index) oTranslateY
    oModify obj_result $ oTranslate .~ (V2 resultX resultY)
    oShow obj_result
    oHide (fst $ obj_array_withOriginalArray!!min_Index) 
    ...
    waitOn $ forkAllWithDifferentLags [0,0,1.5,0,0]
        ...
        ,oAlignRightTo obj_minFound [] obj_funcFullAtEnd 1
        ]
    
    waitOn $ forkAll
        [
        oMoveToNewY obj_result 0 1
        ,oMoveToYAndScale obj_minFound 0.7 0 1
        ]
...
findMinimumAnimation :: Object s a -> Object s Tree -> [(Object s Tree, Integer)] -> Integer -> Scene s ()
findMinimumAnimation _ _ [] _ = do wait 0.0
findMinimumAnimation obj_curMin obj_op  (x:xs) curMin = do
                                                curElementXPos <- oRead (fst x) oCenterX
                                                curElementWidth <- oRead (fst x) oBBWidth
                                                let curCache = curElementXPos
                                                curMinXPos <- oRead obj_curMin oTranslateX
                                                curMinWidth <- oRead obj_curMin oBBWidth
                                                curMin_MaxX <- oRead obj_curMin oRightX
                                                curMinYPos <- oRead obj_curMin oTranslateY
                                                
                                                obj_moreThanQuestion <- oNew moreThanQuestion
                                                oModifyS obj_moreThanQuestion $ oTranslateX .= curMin_MaxX - 0.17 >> oTranslateY .= curMinYPos
                                                oShow obj_moreThanQuestion
                                                
                                                lessThanQuestion_MaxX <- oRead obj_moreThanQuestion oTranslateX
                                                
                                                oModifyS obj_moreThanQuestion $ oScale .= 0
                                                ...
                                                waitOn $ oMoveToNewY (fst x) (curMinYPos) 0.4
                                                waitOn $ oMoveToNewX (fst x) (lessThanQuestion_MaxX + curElementWidth/2 + 0.35) 0.5
                                                
                                                waitOn $ oScaleTo obj_moreThanQuestion 1 0.3
                                                wait 0.5
                                                ...
                                                if (snd x) < curMin
                                                   then do
                                                        waitOn $ forkAllWithDifferentLags [0,1,0]
                                                            [
                                                            oScaleTo obj_moreThanQuestion 0 0.4
                                                            ,oHideWith obj_curMin $ adjustDuration (*1). oFadeOut
                                                            ,oMoveToNewX (fst x) (curMinXPos) 0.6
                                                            
                                                            ]
                                                        findMinimumAnimation (fst x) obj_op xs (snd x)
                                                   else do
                                                        ...
                                                        findMinimumAnimation obj_curMin obj_op xs curMin
                                                return ()
