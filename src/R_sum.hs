...
animation = env $ scene $ do
    ...
    obj_acc <- oNewWithCam (counter 0) cam
    ...
    waitOn $ addTheElementsAnimation obj_acc obj_op obj_array_withOriginalArray 0
    
   
    obj_result <- oNewWithCam (counter $ sum inputArr) cam
...
addTheElementsAnimation :: Object s a -> Object s Tree -> [(Object s Tree, Integer)] -> Integer -> Scene s ()
addTheElementsAnimation _ _ [] _ = do wait 0.0
addTheElementsAnimation obj_acc obj_op (x:xs) curVal = do
                                                ...
                                                waitOn $ forkAllWithLag 0.6
                                                    ...
                                                    ,tweenVar v 1 $ \val -> fromToS 0.0 (fromIntegral $ snd x) . curveS 2 --animate the number roll
                                                    ]
                                                wait 0.1
                                                destroySprite s
                                                ...
                                                addTheElementsAnimation obj_NewAcc obj_op xs (curVal+(snd x))  --iterate again, until the array ends
                                                return ()
